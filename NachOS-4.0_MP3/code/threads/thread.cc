...
Thread::Yield ()
{
    ...
    /*
    MP3
    12/16 modified: Yield is preemptive operation => need not to update burst time
    Instead, just store the burst time + the previous burst time, to ThisTimeBurstTime variable
    */
    double actualBT = kernel->stats->userTicks - this->getStartTime() + this->getThisTimeBurstTime();
    //double estiBT = 0.5 * actualBT + 0.5 * this->getBurstTime();
    //this->setBurstTime(estiBT);
    //record the this time burst time
    this->setThisTimeBurstTime(actualBT);
...
Thread::Sleep (bool finishing)
{
    ...
    /*
    MP3
    12/16 modified: the "only" function that can update burst time
    */
    double actualBT = kernel->stats->userTicks - this->getStartTime() + this->getThisTimeBurstTime();
    //double actualBT = kernel->stats->userTicks - this->getStartTime();
    double estiBT = 0.5 * actualBT + 0.5 * this->getBurstTime();
    this->setBurstTime(estiBT);
    this->setThisTimeBurstTime(0);
