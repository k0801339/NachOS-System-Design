...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            if(execfileNum<9){
                execfile[++execfileNum] = argv[++i];
                tPriority[++priorityNum] = atoi(argv[++i]);
                cout << execfile[execfileNum] <<" "<<tPriority[priorityNum]<< "\n";
            }else{
                cout<<"Exceed the max number of threads. Abort.\n";
                i = i + 2;
            }
...
{
    //MP3
    //threadNum '1' reserved for postal.cc!
    threadNum = 2;
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i], tPriority[i]);
	}
...
{
	return fileSystem->Write(buf, size, id);
}

int Kernel::Read(char *buf, int size, OpenFileId id)
{
	return fileSystem->Read(buf, size, id);
}
int Kernel::Close(OpenFileId id)
{
	return fileSystem->CloseId(id);
}
