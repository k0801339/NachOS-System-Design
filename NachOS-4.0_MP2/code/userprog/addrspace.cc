...
//----------------------------------------------------------------------
/*
Little Endian從最低位元組開始儲存在Memory;Big Endian從最高位元組開始
*/
...
// how big is address space?
    ...
    for(int i=0;i<numPages;i++){
        ...
        int j = 0;
        while(j < NumPhysPages && usedPhysPage[j]==TRUE){
            j++;
        }
        usedPhysPage[j] = TRUE;
...
// Note: this code assumes that virtual address = physical address
	...
	DEBUG(dbgAddr, noffH.code.virtualAddr << ", " << noffH.code.size);
       ...
       int offset = noffH.code.virtualAddr%PageSize;
        executable->ReadAt(
		&(kernel->machine->mainMemory[pageTable[p].physicalPage*PageSize + offset]), noffH.code.size, 
            noffH.code.inFileAddr);
    }
    if (noffH.initData.size > 0) {
        DEBUG(dbgAddr, "Initializing data segment.");
	DEBUG(dbgAddr, noffH.initData.virtualAddr << ", " << noffH.initData.size);
       int p = noffH.initData.virtualAddr/PageSize;
       int offset = noffH.initData.virtualAddr%PageSize;
        executable->ReadAt(
		&(kernel->machine->mainMemory[pageTable[p].physicalPage* PageSize + offset]), noffH.initData.size, 
